...
export default function Header({ navLinks = [], settings }: { navLinks?: NavLink[]; settings?: SiteSettings }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {!showHeader && !isMenuOpen && whatsappUrl && (
          <motion.a
            href={whatsappUrl}
            target="_blank"
                ...
                {!hideQuoteCta ? (
                  <>
                    <QuoteBriefDialog
                      ...
                      triggerClassName={`!hidden h-9 rounded-full px-3.5 text-[0.75rem] font-semibold lg:!inline-flex transition-colors duration-300 ${
                        forceInvert
            ...
            >
              ...
              >
                {[
                  ...
                  { label: tFooter('whatsapp'), url: settings?.whatsapp ? `https://wa.me/${settings.whatsapp.replace(/\D/g, "")}` : undefined },
