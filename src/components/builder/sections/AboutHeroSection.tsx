...
export default function AboutHeroSection({ title, body }: { title: string; body: string }) {
  ...
  return (
    <section className="border-b border-border/55">
      <div className="mx-auto max-w-362.5 px-6 pb-20 pt-[8.6rem] md:px-10 md:pb-28 md:pt-40 lg:px-16 lg:pt-42">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(24rem,0.95fr)] lg:items-end lg:gap-x-16">
          <EditorialWordReveal
            as="h2"
            text={title}
            className="font-serif text-[clamp(2.8rem,5.5vw,5rem)] leading-[0.95] tracking-[-0.05em] text-accent"
          />
          <EditorialReveal delay={0.16} x={30 * dir} y={18}>
            <p className="max-w-152 pb-2 text-[clamp(1.35rem,2.2vw,2.05rem)] leading-[1.28] tracking-[-0.03em] text-foreground/80">
              {body}
            </p>
          </EditorialReveal>
        </div>
