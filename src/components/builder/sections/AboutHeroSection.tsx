...
import { EditorialWordReveal, EditorialReveal } from "@/components/editorial/EditorialMotion";
...
export default function AboutHeroSection({ title, body }: { title: string; body: string }) {
  ...
  return (
    <section className="relative overflow-hidden bg-background">
      ...
      {/* Large ghost numeral — decorative background element */}
      <motion.span
        aria-hidden
      ...
      <div className="relative mx-auto max-w-[1520px] px-6 pb-32 pt-[8rem] md:px-12 md:pb-40 md:pt-48 lg:px-20">
        ...
        >
          <span className="font-mono text-[0.65rem] uppercase tracking-[0.45em] text-accent/25">{t("sideLabel")}</span>
        ...
        <div className="grid grid-cols-1 gap-y-16 lg:grid-cols-[1fr_minmax(0,0.72fr)] lg:gap-x-0">
          ...
          <div className="relative lg:pe-20">
            {/* Offset accent bar behind title */}
            <motion.div
              className="absolute -start-3 top-2 hidden h-[3.2rem] w-[3px] rounded-full bg-accent/30 lg:block"
            ...
            {/* Two-tier underline system */}
            <div className="mt-10 space-y-2.5">
              <motion.div
                className="h-px origin-left bg-accent/25"
                initial={reduced ? false : { scaleX: 0 }}
              ...
              />
              <motion.div
                className="h-px origin-left bg-accent/10"
                style={{ width: "60%" }}
                initial={reduced ? false : { scaleX: 0 }}
                whileInView={{ scaleX: 1 }}
                viewport={{ once: true }}
          ...
          <EditorialReveal delay={0.45} x={22 * dir} y={22}>
            <div className="flex flex-col gap-8 lg:h-full lg:justify-between lg:border-s lg:border-accent/10 lg:ps-14 lg:pt-1">
              ...
              {/* Stat/accent strip at bottom of body col */}
              <motion.div
              ...
              >
                {[t("tagDesign"), t("tagStrategy"), t("tagMotion")].map((tag, i) => (
                  <div key={tag} className="flex flex-col gap-1.5">
                    <div
                      className="h-px bg-accent/30"
                      style={{ width: `${2.5 - i * 0.5}rem` }}
                    />
                    <span className="font-mono text-[0.6rem] uppercase tracking-[0.35em] text-accent/30">
                      {tag}
                    </span>
                  </div>
                ))}
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[0.65rem] uppercase tracking-[0.42em] text-accent/20">
              Create Core
            </span>
            <div className="h-1 w-1 rounded-full bg-accent/25" />
          </div>
