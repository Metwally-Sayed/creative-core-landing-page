...
import ServiceShowcaseSlider from "./ServiceShowcaseSlider";
...
export type ShowcaseImage = { url: string; alt: string };
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1520px] px-6 py-24 md:px-12 md:py-32 lg:px-20">
        ...
        >
          ...
          />
          <span className="font-mono text-[0.65rem] tabular-nums  text-secondary">{num}</span>
        </motion.div>
          ...
          <EditorialReveal delay={0.38} x={20 * dir} y={20}>
            <div className="flex flex-col gap-8 lg:h-full lg:justify-between lg:border-s lg:border-accent/10 lg:ps-14 lg:pt-1">
              ...
              <EditorialReveal delay={0.18} y={14}>
                <Link
                  href="/work"
                  className="group flex items-center gap-3 font-mono text-[0.75rem] uppercase 0.3em] text-accent/60 transition-colors hover:text-accent"
                >
                  ...
                  <Link href={`/projects/${card.slug}`} className="block">
                    ...
                    <div className="mt-4 space-y-1.5">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-mono text-[0.65rem] uppercase 0.35em] text-accent/35">{card.subtitle}</p>
                      </div>
                      <h3 className="max-w-[17ch] font-serif text-[1.9rem] leading-[0.95]  text-accent transition-opacity duration-300 group-hover:opacity-70">
                        {card.title}
                      </h3>
                    </div>
                  </Link>
        ...
        {showcase_images.length > 0 && (
          <EditorialReveal delay={0.2} y={28}>
            <div className="mt-16">
              <ServiceShowcaseSlider images={showcase_images} aspectRatio="16/9" />
            </div>
          </EditorialReveal>
        )}
