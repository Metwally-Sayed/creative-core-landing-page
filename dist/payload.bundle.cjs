...
var import_db_postgres = require("@payloadcms/db-postgres");
...
var payload_config_default = (0, import_payload.buildConfig)({
  ...
  db: (0, import_db_postgres.postgresAdapter)({
    pool: {
  ...
  graphQL: {
    schemaOutputFile: import_path.default.resolve(process.cwd(), "src/graphql-schema.graphql")
  },
  secret: process.env.PAYLOAD_SECRET ?? "hello-monday-payload-dev-secret",
  serverURL: process.env.NEXT_PUBLIC_SERVER_URL ?? "http://localhost:3000",
  typescript: {
    outputFile: import_path.default.resolve(process.cwd(), "src/payload-types.ts")
  }
});
